...
class StaticEmbRNNModel(RNNModel):
    ...
    def __init__(self, *args, language, model_dir="embeddings/wiki.cs.train.suggested.vec"): # , data_dir="data/cze-wikitext"):
        ...
        custom_args[1] = len(vocab_words)
        super(StaticEmbRNNModel, self).__init__(*custom_args)

        weights = torch.FloatTensor(np.vstack(vocab_vecs))
        self.encoder = nn.Embedding.from_pretrained(weights, freeze=False)
        self.word2idx = {w: i for i, w in enumerate(vocab_words)}
        self.idx2word = dict(enumerate(vocab_words))
        self.ntoken = len(vocab_words)

    def synchronize_mapping(self, dictionary: data.Dictionary):
